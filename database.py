...
class KnowledgeRecord(Base):
    ...
    subject_code = Column(String, default="CS301")
    subject_name = Column(String, default="Artificial Intelligence")
    subject_type = Column(String, default="Theory with Lab") # Theory with Lab, Just Theory, Just Lab
    total_marks = Column(Integer, default=100)
    num_internals = Column(Integer, default=2)
    reference_books = Column(Text, default="")
    subject_info = Column(String) # Combined Code + Name
...
class SyllabusModuleModel(Base):
    ...
    module_code = Column(String)      # e.g., "Module 1"
    module_title = Column(String)     # e.g., "Foundations of AI & Search"
    module_hours = Column(Integer, default=10) # Number of Hours for Module
    module_weight = Column(Float, default=20.0) # Stage 1 weight (%)
...
class SyllabusTopicModel(Base):
    ...
    topic_name = Column(String)       # Sub-topic name
    key_concepts = Column(Text, default="") # Key concepts under sub-topic
    topic_weight = Column(Float, default=5.0) # Stage 2 weight (%)
    ...
    try:
        ...
        subjects_data = [
            ...
            ("faculty2", "Pragyan Institute of Technology, Bengaluru", "Department of Information Technology", 
             "IT405: Cloud Computing & Distributed Systems", "Theory with Lab", 100, 2, "1. Tanenbaum - Distributed Systems",
            ...
            ("faculty3", "Pragyan Institute of Technology, Bengaluru", "Department of Artificial Intelligence & Data Science", 
             "AIDS501: Generative AI & Large Language Models", "Just Theory", 100, 2, "1. Transformers Handbook",
            ...
            ("faculty5", "Karnataka School of Engineering, Mysuru", "Department of Information Technology", 
             "CS202: Advanced Data Structures & Algorithms", "Theory with Lab", 100, 2, "1. Cormen - Introduction to Algorithms",
            ...
            ("faculty8", "Pragyan Institute of Technology, Bengaluru", "Department of Cybersecurity & Defense", 
             "CY310: Cyber Security & Ethical Hacking", "Theory with Lab", 100, 2, "1. William Stallings - Cryptography & Network Security",
            ...
            ("faculty6", "Pragyan Institute of Technology, Bengaluru", "Department of Electronics & Communication Engineering", 
             "ECE401: Digital Signal Processing & Computer Vision", "Theory with Lab", 100, 2, "1. Proakis - Digital Signal Processing",
            ...
            ("faculty4", "Karnataka School of Engineering, Mysuru", "Department of Computer Science & Engineering", 
             "CSE403: Internet of Things & Embedded Systems", "Theory with Lab", 100, 2, "1. Raj Kamal - Embedded Systems",
            ...
            ("faculty9", "Silicon Valley Tech University, Hubli", "Department of Information Technology", 
             "IT304: Big Data Analytics & NoSQL Databases", "Theory with Lab", 100, 2, "1. Tom White - Hadoop: The Definitive Guide",
            ...
            ("faculty7", "Silicon Valley Tech University, Hubli", "Department of Computer Science & Engineering", 
             "CSE502: Distributed Ledger & Blockchain Technology", "Just Theory", 100, 2, "1. Arvind Narayanan - Bitcoin and Cryptocurrency Technologies",
            ...
            ("faculty10", "Karnataka School of Engineering, Mysuru", "Department of Electronics & Communication Engineering", 
             "ECE302: VLSI Design & Microelectronics", "Theory with Lab", 100, 2, "1. Kang & Leblebici - CMOS Digital Integrated Circuits",
        ...
        for fac_uname, college, dept, subj_info, stype, tmarks, internals, books, topics, syl_text in subjects_data:
            code, name = subj_info.split(": ", 1) if ": " in subj_info else ("CS101", subj_info)
...
def save_modular_syllabus(knowledge_id: int, modules_dict: dict):
    db = SessionLocal()
    try:
        db.query(SyllabusModuleModel).filter(SyllabusModuleModel.knowledge_record_id == knowledge_id).delete()
        
        for m_code, m_data in modules_dict.items():
            db_mod = SyllabusModuleModel(
                knowledge_record_id=knowledge_id,
                module_code=m_code,
                module_title=m_data.get("title", ""),
                module_hours=int(m_data.get("hours", 10)),
                module_weight=float(m_data.get("weight", 20.0))
            )
            db.add(db_mod)
            db.flush()
            
            subtopics = m_data.get("subtopics", {})
            for st_name, st_concepts in subtopics.items():
                db_top = SyllabusTopicModel(
                    module_id=db_mod.id,
                    topic_name=st_name,
                    key_concepts=st_concepts,
                    topic_weight=5.0
                )
                db.add(db_top)
        db.commit()
        ...
        db.rollback()
        print(f"Error saving modular syllabus: {e}")
        return False
    finally:
        db.close()

def get_modular_syllabus(knowledge_id: int):
    db = SessionLocal()
    try:
        mods = db.query(SyllabusModuleModel).filter(SyllabusModuleModel.knowledge_record_id == knowledge_id).all()
        result = {}
        for m in mods:
            sub_dict = {t.topic_name: t.key_concepts for t in m.topics}
            result[m.module_code] = {
                "title": m.module_title,
                "hours": m.module_hours,
...
def hybrid_rag_llm_query(query: str, knowledge_id: int):
    db = SessionLocal()
    rec = db.query(KnowledgeRecord).filter(KnowledgeRecord.id == knowledge_id).first()
    db.close()
    
    structured_hierarchy = get_modular_syllabus(knowledge_id)
    structured_context = f"Subject: {rec.subject_info if rec else 'N/A'}\nStructured Curriculum & Modular Breakdown:\n"
    for m_code, data in structured_hierarchy.items():
        structured_context += f"- {m_code}: {data['title']} (Hours: {data['hours']} hrs)\n"
        for st_name, st_con in data['subtopics'].items():
            structured_context += f"   * {st_name} -> Concepts: {st_con}\n"
